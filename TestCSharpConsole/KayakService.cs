...
internal static class KayakService
{
    public static void GetSearchResults(string originItaCode, string destinationItaCode, DateOnly departureDate)
    {
        ...
        // Find div elements of class type: nrc6-mod-pres-multi-fare
        var fareElements =  driver.FindElements(By.ClassName("nrc6-mod-pres-multi-fare"));

        // Extract the price and booking URL for each fare
        foreach (var fareElement in fareElements)
        {
            ...
            var durationText = fareElement.FindElement(By.XPath(".//div[@class='xdW8']//div[contains(@class, 'vmXl-mod-variant-default')]")).Text;
            // Extract the carrier data
            var carrierText = fareElement.FindElement(By.XPath(".//div[@class='VY2U']//div[contains(@class, 'c_cgF-mod-variant-default')]")).Text;

            // Extract the price data
            var priceElement = fareElement.FindElement(By.ClassName("f8F1-price-text"));
            var priceText = priceElement.Text;

            // Print the extracted data
            Console.WriteLine($"Price: {priceText} ][ Time: {durationText} from {flightTimesText} ][ Carrier: {carrierText}");
